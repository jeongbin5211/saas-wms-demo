...
로그인 전 메인 화면은 단순 로그인 페이지가 아니라, 서비스 소개와 시연 진입을 제공하는 외부 메인 페이지로 구성한다.
메인 화면에서는 실제 SaaS 서비스처럼 보이는 톤을 유지하고, 기술 설명과 시연 안내는 별도 탭에서 자연스럽게 제공한다.
...
- `/about`, `/features`, `/logistics`, `/demo-guide`, `/tech`는 상단 메뉴별 독립 소개 화면으로 구성한다.
...
- 화면에 노출되는 텍스트는 기술 고유명사를 제외하고 한글을 우선 사용한다.
...
품목정보는 `ItemMaster -> ItemClass -> Item` 계층으로 관리한다.
...
재고는 `Item + Location` 조합으로 현재 수량을 관리한다. 재고 변경은 반드시 `inventory_histories`에 이력을 남기는 방향으로 설계한다.

```text
Inventory
- quantity: 전체 재고 수량
- allocated_quantity: 출고 예정으로 할당된 수량
- available_quantity: 출고 가능 수량, API 응답에서 quantity - allocated_quantity로 계산
```
